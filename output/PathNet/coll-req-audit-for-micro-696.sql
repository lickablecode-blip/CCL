/*
* Name:     Coll_Req_Audit for Micro -696
* Source:   Inbox/PathNet/Coll_Req_Audit for Micro -696.txt
* Purpose:
* Imported: 2026-05-15
* Category: PathNet  (reason: subfolder)
* Lines:    32
* Notes:
*/

select 
	oc.primary_mnemonic,
	pst_exists = decode(pst.seq,"Y","N"), 
	pst_accession_class_disp = uar_get_code_display( pst.accession_class_cd ),
	pst_specimen_type_disp = uar_get_code_display( pst.specimen_type_cd ),
	pst_default_collection_method_disp = uar_get_code_display( pst.default_collection_method_cd ),
	ciq_exists = decode(ciq.seq,"Y","N"),
	ciq.sequence,
	ciq.service_resource_cd,
	ciq_service_resource_disp = uar_get_code_display( ciq.service_resource_cd ),
	ciq.age_from_minutes,
	ciq.age_to_minutes,
	ciq.min_vol,
	ciq.min_vol_units,
	ciq_spec_cntnr_disp = uar_get_code_display( ciq.spec_cntnr_cd ),
	ciq_coll_class_disp = uar_get_code_display( ciq.coll_class_cd ),
	ciq_spec_hndl_disp = uar_get_code_display( ciq.spec_hndl_cd )

from
	order_catalog  oc,
	procedure_specimen_type  pst,
	collection_info_qualifiers  ciq

plan oc
where oc.active_ind = 1
  and oc.activity_type_cd = 696

join pst
where pst.catalog_cd = oc.catalog_cd

join ciq
where ciq.catalog_cd = pst.catalog_cd
  and ciq.specimen_type_cd = pst.specimen_type_cd

order by	oc.primary_mnemonic,
			pst_specimen_type_disp,
			ciq.sequence
