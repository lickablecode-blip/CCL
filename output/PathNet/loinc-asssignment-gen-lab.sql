/*
* Name:     LOINC asssignment Gen Lab
* Source:   Inbox/PathNet/LOINC asssignment Gen Lab.txt
* Purpose:
* Imported: 2026-05-15
* Category: PathNet  (reason: subfolder)
* Lines:    18
* Notes:
*/

SELECT
	D.TASK_ASSAY_CD
	, ACTIVITY_TYPE = UAR_GET_CODE_DISPLAY(D.ACTIVITY_TYPE_CD)
	, DTA_MNEMONIC = D.MNEMONIC
	, DTA_DESCRIPTION = D.DESCRIPTION
	, SPECIMEN_TYP = UAR_GET_CODE_DISPLAY(C.SPECIMEN_TYPE_CD)
	, SERVICE_RESOURCE = UAR_GET_CODE_DISPLAY(C.SERVICE_RESOURCE_CD)
	, CONCEPT_CKI_GEN_LAB = C.CONCEPT_CKI
	, C.CONCEPT_IDENTIFIER_DTA_ID

FROM
	DISCRETE_TASK_ASSAY   D
	, CONCEPT_IDENTIFIER_DTA   C

PLAN d where D.ACTIVITY_TYPE_CD = 692   and d.active_ind = 1
join c where C.TASK_ASSAY_CD = outerjoin(D.TASK_ASSAY_CD)

ORDER BY
	ACTIVITY_TYPE
	, D.MNEMONIC

WITH NOCOUNTER, SEPARATOR=" ", FORMAT
