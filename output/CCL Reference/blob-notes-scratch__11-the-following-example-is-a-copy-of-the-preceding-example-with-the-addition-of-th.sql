 ...
 * Anchor       : The following example is a copy of the preceding example with the addition of the CLINICAL_EVENT table:
 * Block index  : 11 of 11
 * Detected lang: ccl
 * Lines        : 170
 *
...
drop program
1_ccl_multi_multi_blob_ce_cb go
create
program 1_ccl_multi_multi_blob_ce_cb
prompt
"Output
...
;the following qualification gets only the valid rows and prevents
getting duplicates from the CE_BLOB table
and ce.VALID_FROM_DT_TM < cnvtdatetime(curdate,curtime3)
and ce.VALID_UNTIL_DT_TM > cnvtdatetime(curdate,curtime3)
join cb where
ce.event_id = cb.event_id
...
order by
ce.encntr_id, cb.event_id, cb.blob_seq_num
head
cb.event_id
...
assigned to the blobout variable.
;;;***** additional processing on the uncompressed blob can be done
using the blobout variable
...
;***** use uar_rtf2 to strip the rtf from the blob. The uncompressed
blob without the rtf is assigned to BlobNoRTF
